#> With destroy_roots off the mangrove is chopped but its 6 roots stay
# @environment timber_test:destroy_roots_off
...
await delay 1s
function timber_test:count_d7 {test:"settings_destroy_roots_off_keeps_roots"}
function timber_test:player {name:"roots_off_t",axe:"minecraft:iron_axe"}
dummy roots_off_t mine ~20 ~1 ~20
...
function timber_test:assert_roots_left {n:6}
function timber_test:assert_no_tree {test:"settings_destroy_roots_off_keeps_roots"}

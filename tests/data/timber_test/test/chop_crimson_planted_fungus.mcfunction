#> Mining the bottom stem of a crimson fungus grown from a planted one with an axe chops down the whole fungus (chop_fungi is on)
...
function timber_test:tree {feature:"minecraft:crimson_fungus_planted"}
...
# wait until the trees are done and chop one after the other, 24 ticks after the first fungus test.
await delay 124t
function timber_test:player {name:"crimson_pl_t",axe:"minecraft:iron_axe"}
function timber_test:mine {name:"crimson_pl_t"}
...
function timber_test:assert_no_fungus {test:"chop_crimson_planted_fungus"}

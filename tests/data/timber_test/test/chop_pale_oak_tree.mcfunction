#> Mining the bottom log of a 2x2 pale oak tree with an axe chops down the whole tree
...
function timber_test:tree {feature:"minecraft:pale_oak"}
assert block ~20 ~ ~20 #minecraft:logs

# let the leaves settle before the tree is chopped
await delay 1s
function timber_test:count_d7 {test:"chop_pale_oak_tree"}

function timber_test:player {name:"pale_oak_t",axe:"minecraft:iron_axe"}
function timber_test:mine {name:"pale_oak_t"}
...
function timber_test:assert_no_logs_warn_leaves {test:"chop_pale_oak_tree"}

#> Mining the bottom log of a spruce tree with an axe chops down the whole tree
# @template timber_test:arena
# @timeout 200
# @skyaccess true

function timber_test:platform
function timber_test:tree {feature:"minecraft:spruce"}
assert block ~20 ~ ~20 #minecraft:logs

# let the leaves settle before the tree is chopped
await delay 1s
function timber_test:count_d7 {test:"chop_spruce_tree"}

function timber_test:player {name:"spruce_t",axe:"minecraft:iron_axe"}
function timber_test:mine {name:"spruce_t"}
...
function timber_test:assert_no_tree {test:"chop_spruce_tree"}

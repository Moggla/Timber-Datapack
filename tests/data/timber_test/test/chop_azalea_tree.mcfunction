#> Mining the bottom log of an azalea tree with an axe chops down the whole tree
# @template timber_test:arena
# @timeout 200
# @skyaccess true

function timber_test:platform
function timber_test:tree {feature:"minecraft:azalea_tree"}
assert block ~20 ~ ~20 #minecraft:logs

# let the leaves settle before the tree is chopped
await delay 1s
function timber_test:count_d7 {test:"chop_azalea_tree"}

function timber_test:player {name:"azalea_t",axe:"minecraft:iron_axe"}
function timber_test:mine {name:"azalea_t"}
...
function timber_test:assert_no_tree {test:"chop_azalea_tree"}

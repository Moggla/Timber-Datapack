#> With wear_out off only the mined block damages the axe (vanilla takes 1)
# @environment timber_test:wear_out_off
...
await delay 1s
function timber_test:player {name:"wear_off_t",axe:"minecraft:iron_axe"}
dummy wear_off_t mine ~20 ~0 ~20
...
execute store result score #dmg timber_test run data get entity @e[type=minecraft:player,name=wear_off_t,limit=1] SelectedItem.components."minecraft:damage"

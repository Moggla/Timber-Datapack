#> Mining the bottom log of a 2x2 pine tree with an axe chops down the whole tree
...
function timber_test:tree {feature:"minecraft:mega_pine"}
assert block ~20 ~ ~20 #minecraft:logs

# let the leaves settle before the tree is chopped
await delay 1s
function timber_test:count_d7 {test:"chop_mega_pine_tree"}

function timber_test:player {name:"mega_pine_t",axe:"minecraft:iron_axe"}
function timber_test:mine {name:"mega_pine_t"}
...
function timber_test:assert_no_logs_warn_leaves {test:"chop_mega_pine_tree"}

#> With Unbreaking III the axe wears less than half as much as there are logs (about a quarter)
...
await delay 1s
function timber_test:count_logs {test:"enchantments_unbreaking_lowers_the_wear"}
function timber_test:player {name:"unb_three_t",axe:"minecraft:iron_axe"}
item replace entity @e[type=minecraft:player,name=unb_three_t,limit=1] weapon.mainhand with minecraft:iron_axe[minecraft:enchantments={"minecraft:unbreaking":3}]
function timber_test:mine {name:"unb_three_t"}
...
execute store result score #dmg timber_test run data get entity @e[type=minecraft:player,name=unb_three_t,limit=1] SelectedItem.components."minecraft:damage"
...
scoreboard players operation #dmg timber_test -= #logs0_enchantments_unbreaking_lowers_the_wear timber_test
assert score #dmg timber_test matches ..-1

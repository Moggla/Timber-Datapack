#> Mining the bottom stem of a crimson fungus with an axe chops down the whole fungus (chop_fungi is on)
...
# wait until the trees are done and chop one after the other, 0 ticks after the first fungus test.
await delay 100t
function timber_test:player {name:"crimson_t",axe:"minecraft:iron_axe"}
function timber_test:mine {name:"crimson_t"}
...
function timber_test:assert_no_fungus {test:"chop_crimson_fungus"}

#> With standing off and sneaking on a sneaking player chops
# @environment timber_test:sneak_only
...
await delay 1s
function timber_test:count_d7 {test:"settings_sneak_only_sneaking_chops"}
function timber_test:player {name:"sneakon_t",axe:"minecraft:iron_axe"}
dummy sneakon_t sneak true
# the dummy counts as sneaking a tick or more after the command: wait for it
await entity @e[type=minecraft:player,name=sneakon_t,predicate=timber:sneaking]
dummy sneakon_t mine ~20 ~0 ~20
...
function timber_test:assert_no_tree {test:"settings_sneak_only_sneaking_chops"}

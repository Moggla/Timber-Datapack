#> remove scoreboards
scoreboard objectives remove timber_w_axe
scoreboard objectives remove timber_s_axe
scoreboard objectives remove timber_i_axe
scoreboard objectives remove timber_g_axe
scoreboard objectives remove timber_d_axe
scoreboard objectives remove timber_sneak_t
scoreboard objectives remove timber
# old
scoreboard objectives remove t_wooden_axe
scoreboard objectives remove t_stone_axe
scoreboard objectives remove t_iron_axe
scoreboard objectives remove t_golden_axe
scoreboard objectives remove t_diamond_axe
scoreboard objectives remove t_sneak_time

tellraw @a [{"text":"Timber Datapack uninstalled :(","color":"dark_red"},{"text":"\n[click here to leave me a feedback]","color":"gold","clickEvent":{"action":"open_url","value":"https://www.planetminecraft.com/mod/timber-datapack/"}}]
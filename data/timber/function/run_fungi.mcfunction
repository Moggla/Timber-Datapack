...
# if fungi is valid
# a fungus has far fewer cap blocks next to its stem than a tree has leaves (warped fungus and huge mushrooms down to 5), so it has its own minimum
execute if score leaves_found timber >= min_caps_found timber unless score tree_size timber matches 0 run scoreboard players set tree_type timber 2
